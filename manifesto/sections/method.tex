...
The first is to analyse the raw results output by the simulation to determine the value for each of the objectives by which the simulations are to be judged.
...
%\claudio{In the prev par, make it clear what example we are referrign to. It seems that the reader has to imagine an example to make sense of what is given.}

The final step in a closed loop is to rank the designs according to how well each performs.
The ranking may be trivial, such as in a search for a design that minimises the total amount of energy used, or it may be more complex if there are multiple objectives to optimise and trade off.
...
%\claudio{I think the previous subsection is detailed (in comparison with the others). Maybe it should be summarized, with references given to other introductory material.}
...
%\claudio{Maybe we should give this sectino a different name. I like Workflows better. Currently, it seems that this is sort of the solution provided by into-cps, and not guidelines (as the main section implies).}
