...
The Agriculture case study led by the Danish company Agrointelli concerns both an automated control system for an agricultural robot as well as an autonomously operating lawn mower. The robot, which provides more efficient removal of weeds in the field while operating safely but \claudio{and?} with minimal human interaction. In addition, the development of the autonomous control for the lawn mower was carried out. The challenge in both cases was to simulate the behaviour of physical components (such as mechanical loading on certain elements) together with controls of the automated system even before the physical mechanical components are available. These controls access local data (e.g.\ sensors) and external data (e.g.\ GPS). Model-based design allowed for accelerated time-to-market and virtual verification while reducing the need for multiple physical prototypes.
...
\claudio{This might be a too strong claim. One cannot prove anything on black boxes. Maybe weaken the claim a bit by saying ``might''. Or give me a paper where this is done, as I'm very interested!}
...
\claudio{I think this section is lacking a table (or figure) summarizing how the features of into-cps help on the challenges presented in the previous section. Moreover, on that same table (or figure), you could add which features where used in which case study. For example, see Fig \ref{figure:sketch}.
Then, we could put in bold the features in the corresponding sections, to make the text more scannable (as the doc is quite long, I wouldn't be surprised that a lot of the readers are not going to read it in detail)}
